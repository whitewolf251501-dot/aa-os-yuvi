...
function initDashboard(){
  ...
  if(typeof v7RenderLiveActivity==='function')v7RenderLiveActivity();
  if(typeof v7RenderUpcomingEvents==='function')v7RenderUpcomingEvents();
  if(typeof v7RenderUptime==='function')v7RenderUptime();
  if(typeof v7RenderMachineSparkline==='function')v7RenderMachineSparkline();
  updateStats();checkReminders();
  if(typeof v7ComputeAndRenderHealthScore==='function')v7ComputeAndRenderHealthScore();
...
function nav(viewId,navEl,mobileEl){
  ...
  // v7 — digital rain only animates while Command view is the active view
  window.__v7CommandActive=(viewId==='command');
}
...
// v7.5 — item 3: LIVE ACTIVITY. Distinct from the widget-generation
// Recent Activity feed above — this logs real CRM events: new leads,
// pipeline stage moves, payment status changes. Nothing fabricated.
var V7_LIVE_ACTIVITY_KEY='yuvi_live_activity';
function v7LogLiveActivity(type,text){
  var list=safeParseLS(V7_LIVE_ACTIVITY_KEY,[]);
  list.push({type:type,text:text,time:new Date().toISOString()});
  list=list.slice(-30);
  localStorage.setItem(V7_LIVE_ACTIVITY_KEY,JSON.stringify(list));
  v7RenderLiveActivity();
}
function v7RenderLiveActivity(){
  var el=document.getElementById('v7-live-activity-list');
  if(!el)return;
  var list=safeParseLS(V7_LIVE_ACTIVITY_KEY,[]).slice().reverse().slice(0,8);
  if(!list.length){el.innerHTML='<div style="font-size:9px;color:var(--muted);">No live events yet.</div>';return;}
  el.innerHTML=list.map(function(a){
    return '<div class="v7-live-item"><div class="v7-live-dot '+(a.type||'')+'"></div>'
      +'<div class="v7-live-text">'+escHtml(a.text)+'</div>'
      +'<div class="v7-live-time">'+timeAgo(a.time)+'</div></div>';
  }).join('');
}
setInterval(v7RenderLiveActivity,60000);

// v7.5 — item 4: UPCOMING EVENTS. No fabricated calendar — derived from
// real live state: proposal-sent deadlines (stage entry + stale-threshold
// setting), advance-pending follow-ups, and overdue/pending client payments.
function v7GetUpcomingEvents(){
  var events=[];
  var thresholdDays=(typeof getStageThresholdDays==='function')?getStageThresholdDays():5;
  var DAY=86400000;
  pipeline.forEach(function(p){
    if(p.stage==='proposal_sent'){
      var enteredAt=new Date(p.stageEnteredAt||p.lastTouched||Date.now()).getTime();
      events.push({title:'Proposal Deadline \u2014 '+p.name,type:'deadline',icon:'\u23F3',date:enteredAt+(thresholdDays*DAY)});
    }
    if(p.stage==='advance_pending'){
      events.push({title:'Advance Payment Follow-up \u2014 '+p.name,type:'payment',icon:'\u20B9',date:Date.now()});
    }
  });
  clients.forEach(function(c){
    if(c.payment==='overdue'){
      events.push({title:'Payment Follow-up \u2014 '+c.name,type:'payment',icon:'\u20B9',date:Date.now()-DAY});
    }else if(c.payment==='pending'){
      events.push({title:'Client Call \u2014 '+c.name,type:'call',icon:'\u260E',date:Date.now()});
    }
  });
  events.sort(function(a,b){return a.date-b.date;});
  return events.slice(0,6);
}
function v7FormatEventWhen(ts){
  var d=new Date(ts);
  var diffDays=Math.round((ts-Date.now())/86400000);
  var dateStr=d.toLocaleDateString('en-IN',{day:'2-digit',month:'short'})+', '+d.toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'});
  if(diffDays<0)return {label:'Overdue \u2014 '+dateStr,cls:'overdue'};
  if(diffDays===0)return {label:'Today, '+d.toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'}),cls:''};
  if(diffDays===1)return {label:'Tomorrow, '+d.toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'}),cls:''};
  return {label:dateStr,cls:''};
}
function v7RenderUpcomingEvents(){
  var el=document.getElementById('v7-upcoming-events-list');
  if(!el)return;
  var events=v7GetUpcomingEvents();
  if(!events.length){el.innerHTML='<div style="font-size:9px;color:var(--muted);">No upcoming events \u2014 pipeline and payments are clear.</div>';return;}
  el.innerHTML=events.map(function(e){
    var w=v7FormatEventWhen(e.date);
    return '<div class="v7-event-item"><div class="v7-event-icon">'+e.icon+'</div>'
      +'<div class="v7-event-body"><div class="v7-event-title">'+escHtml(e.title)+'</div>'
      +'<div class="v7-event-when '+w.cls+'">'+w.label+'</div></div></div>';
  }).join('');
}
...
function cycleRevStatus(id){var r=revenueData.find(function(x){return x.id===id;});if(!r)return;r.status={paid:'pending',pending:'overdue',overdue:'paid'}[r.status]||'paid';renderRevenue();if(typeof v7LogLiveActivity==='function')v7LogLiveActivity('payment',r.name+' revenue \u2192 '+r.status.toUpperCase());}
...
function renderResearchCard(resDiv,l,id,data,nameEnc,cityEnc){
  ...
  resDiv.innerHTML='<div class="research-card">'
    ...
    +'<div style="font-family:var(--mono);font-size:8px;color:var(--text);line-height:1.7;padding:7px;background:rgba(61,125,255,.04);border-left:2px solid var(--blade);margin-bottom:8px;">'+(data.opening_line?escHtml(data.opening_line):'—')+'</div>'
...
function renderOutreachLeadList(){
  ...
  list.innerHTML=leads.filter(function(l){return l.phone&&l.phone.length>=10;}).map(function(l){
    ...
    return '<div style="display:flex;align-items:center;gap:8px;padding:5px 7px;border:1px solid var(--edge2);background:rgba(61,125,255,.02);">'
...
function movePipeStage(id,stage){var c=pipeline.find(function(x){return x.id===id;});if(!c)return;c.stage=stage;c.lastTouched=new Date().toISOString();c.stageEnteredAt=new Date().toISOString();localStorage.setItem('yuvi_pipeline',JSON.stringify(pipeline));renderPipeline();renderPipeDetail(c);showToast(c.name+' \u2192 '+PIPE_LABELS[stage]);if(typeof v7LogLiveActivity==='function')v7LogLiveActivity('stage',c.name+' moved to '+PIPE_LABELS[stage]);offerNextAction('pipeline_stage',{name:c.name,stage:stage});}
...
function renderClients(){
  ...
  if(typeof v7RenderClientsSummary==='function')v7RenderClientsSummary();
  if(typeof v7ComputeAndRenderHealthScore==='function')v7ComputeAndRenderHealthScore();
  if(typeof v7RenderUpcomingEvents==='function')v7RenderUpcomingEvents();
}
...
function cycleClientPayment(cid){var c=clients.find(function(x){return x.id===cid;});if(!c)return;c.payment={paid:'pending',pending:'overdue',overdue:'paid'}[c.payment]||'paid';localStorage.setItem('yuvi_clients',JSON.stringify(clients));renderClients();showToast(c.name+' \u2192 '+c.payment.toUpperCase());if(typeof v7LogLiveActivity==='function')v7LogLiveActivity('payment',c.name+' payment \u2192 '+c.payment.toUpperCase());}
...
/* =========================================================
   YUVI v7 — ADDITIVE LAYER
   Nothing above this point is modified. Everything below is
   new, reads from existing data where possible, and degrades
   gracefully if n8n is offline (dashboard stays 100% usable
   on Groq alone).
   ========================================================= */

/* ---- Workspace (n8n) status polling ---- */
var v7WorkspaceOnline=false;
function v7CheckWorkspace(){
  if(!window.YuviN8N){v7SetWorkspaceStatus(false);return;}
  window.YuviN8N.pingWorkspace().then(function(online){
    v7SetWorkspaceStatus(online);
    if(online)window.YuviN8N.syncQueueWhenOnline();
  }).catch(function(){v7SetWorkspaceStatus(false);});
}
function v7SetWorkspaceStatus(online){
  v7WorkspaceOnline=online;
  var pill=document.getElementById('v7-workspace-pill');
  var txt=document.getElementById('v7-workspace-text');
  if(pill&&txt){
    pill.classList.toggle('online',online);
    txt.textContent='WORKSPACE: '+(online?'ONLINE':'OFFLINE');
  }
  // v7.5 — item 6: real online-since tracking (not a fake counter).
  // Only stamps a fresh "online since" when transitioning offline->online.
  var wasOnline=localStorage.getItem('yuvi_workspace_was_online')==='1';
  if(online&&!wasOnline){
    localStorage.setItem('yuvi_workspace_online_since',new Date().toISOString());
  }
  if(!online){
    localStorage.removeItem('yuvi_workspace_online_since');
  }
  localStorage.setItem('yuvi_workspace_was_online',online?'1':'0');
  var dot=document.getElementById('v7-mw-dot');
  var statusEl=document.getElementById('v7-mw-status');
  if(dot)dot.classList.toggle('online',online);
  if(statusEl)statusEl.textContent=online?'ONLINE':'OFFLINE';
  v7RenderUptime();
}
function v7RenderUptime(){
  var el=document.getElementById('v7-mw-uptime');
  if(!el)return;
  var since=localStorage.getItem('yuvi_workspace_online_since');
  if(!since){el.textContent='OFFLINE';return;}
  var ms=Date.now()-new Date(since).getTime();
  var mins=Math.floor(ms/60000);
  var h=Math.floor(mins/60),m=mins%60;
  el.textContent=(h>0?h+'h ':'')+m+'m';
}
setInterval(v7RenderUptime,30000);
function v7RenderMachineSparkline(){
  var el=document.getElementById('v7-mw-sparkline');
  if(!el)return;
  var q=v7GetQueue();
  var now=Date.now();var HOUR=3600000;
  var buckets=new Array(8).fill(0);
  q.forEach(function(t){
    var age=now-new Date(t.createdAt||now).getTime();
    var idx=7-Math.min(7,Math.floor(age/HOUR));
    if(idx>=0&&idx<8)buckets[idx]++;
  });
  var max=Math.max.apply(null,buckets)||1;
  var w=140,h=26,step=w/(buckets.length-1);
  var pts=buckets.map(function(v,i){return (i*step).toFixed(1)+','+(h-(v/max)*h).toFixed(1);}).join(' ');
  el.innerHTML='<svg viewBox="0 0 '+w+' '+h+'" xmlns="http://www.w3.org/2000/svg"><polyline points="'+pts+'" fill="none" stroke="var(--v7-violet,#a78bfa)" stroke-width="1.4"/></svg>';
}
setInterval(v7CheckWorkspace,20000);

/* ---- Task Queue (local-first; n8n executes when online) ---- */
function v7GetQueue(){try{return JSON.parse(localStorage.getItem('yuvi_n8n_queue')||'[]');}catch(e){return [];}}
function v7SaveQueue(q){localStorage.setItem('yuvi_n8n_queue',JSON.stringify(q));}
function v7EnqueueTask(task){
  var q=v7GetQueue();
  q.push(Object.assign({id:Date.now(),status:task.requiresApproval?'awaiting_approval':'queued',createdAt:new Date().toISOString()},task));
  v7SaveQueue(q);
  v7RenderTeamTab();
}
function v7ApproveTask(id){
  var q=v7GetQueue();
  var t=q.find(function(x){return x.id===id;});
  if(t)t.status='queued';
  v7SaveQueue(q);
  v7RenderTeamTab();
  if(v7WorkspaceOnline&&window.YuviN8N)window.YuviN8N.syncQueueWhenOnline();
}

/* ---- AI Team floor (agent cards) — status derived from real queue state ---- */
var V7_AGENTS=[
  {id:'scout',name:'Scout',role:'Lead Hunter'},
  {id:'sherlock',name:'Sherlock',role:'Research Analyst'},
  {id:'spark',name:'Spark',role:'Content Creator'},
  {id:'ledger',name:'Ledger',role:'Ops & Finance'},
  {id:'echo',name:'Echo',role:'Client Success'}
];
/* ---- Workflows (item 5) — read-only list of repeatable multi-step
   processes, tied to the same task-queue data model (matched by
   task.type). No real n8n execution here — this is the frontend
   compat scaffold per BUILD_PLAN_v7.md; counts reflect real local queue state. ---- */
var V7_WORKFLOWS=[
  {id:'lead_gen',name:'Lead Generation Sweep',agent:'scout',steps:['Scan target category/area','Score & dedupe leads','Push to Leads view queued for outreach']},
  {id:'competitor_research',name:'Competitor Research',agent:'sherlock',steps:['Pull competitor list for niche','Summarize pricing & positioning','Attach findings to knowledge base']},
  {id:'content_batch',name:'Content Batch Creation',agent:'spark',steps:['Draft post/reel captions for the week','Queue for review','Mark delivered in client package items']},
  {id:'monthly_report',name:'Monthly Client Report',agent:'ledger',steps:['Pull revenue & task data for client','Generate report draft','Send for approval before delivery']},
  {id:'client_checkin',name:'Client Check-in & Retention',agent:'echo',steps:['Flag clients with no recent touch','Draft check-in message','Log outcome to client notes']}
];
function v7RenderWorkflows(){
  var el=document.getElementById('v7-team-workflows');
  if(!el)return;
  var q=v7GetQueue();
  el.innerHTML=V7_WORKFLOWS.map(function(w){
    var agent=V7_AGENTS.find(function(a){return a.id===w.agent;});
    var matched=q.filter(function(t){return t.type===w.id;});
    var active=matched.filter(function(t){return t.status==='queued'||t.status==='working'||t.status==='awaiting_approval';}).length;
    var done=matched.filter(function(t){return t.status==='done';}).length;
    return '<div class="v7-workflow-card">'
      +'<div class="v7-workflow-head"><span class="v7-workflow-name">'+escHtml(w.name)+'</span>'
      +'<span class="v7-workflow-agent">'+(agent?agent.name:w.agent)+'</span></div>'
      +'<div class="v7-workflow-steps">'+w.steps.map(function(s,i){return'<span class="v7-workflow-step">'+(i+1)+'. '+escHtml(s)+'</span>';}).join('')+'</div>'
      +'<div class="v7-workflow-meta">'+active+' active &middot; '+done+' completed</div>'
      +'</div>';
  }).join('');
}
function v7RenderTeamTab(){
  var floor=document.getElementById('v7-team-floor');
  var queueEl=document.getElementById('v7-team-queue');
  var logsEl=document.getElementById('v7-team-logs');
  if(!floor)return;
  var q=v7GetQueue();
  floor.innerHTML=V7_AGENTS.map(function(a){
    var tasks=q.filter(function(t){return t.agent===a.id&&(t.status==='queued'||t.status==='working');});
    var status=tasks.length?'working':'idle';
    var pct=tasks.length?Math.min(90,20+tasks.length*20):0;
    return '<div class="v7-agent-card">'
      +'<div class="v7-agent-avatar">'+a.name.charAt(0)+'</div>'
      +'<div class="v7-agent-name">'+a.name+'</div>'
      +'<div class="v7-agent-status '+status+'">'+status.toUpperCase()+'</div>'
      +'<div class="v7-agent-bar"><div class="v7-agent-fill" style="width:'+pct+'%;"></div></div>'
      +'<div class="v7-agent-eta">'+(tasks.length?tasks.length+' task(s) queued':'no tasks queued')+'</div>'
      +'</div>';
  }).join('');
  if(queueEl)queueEl.innerHTML=q.length?q.map(function(t){
    return '<div class="v7-task-row'+(t.status==='awaiting_approval'?' approval':'')+'">'
      +'<span>'+escHtml(t.label||t.type||'Task')+' &middot; '+t.status.toUpperCase().replace('_',' ')+'</span>'
      +(t.status==='awaiting_approval'?'<button class="v7-approve-btn" onclick="v7ApproveTask('+t.id+')">APPROVE</button>':'')
      +'</div>';
  }).join(''):'<div style="font-family:var(--mono);font-size:8px;color:var(--muted);padding:8px;">No tasks queued.</div>';
  if(logsEl)logsEl.innerHTML=q.filter(function(t){return t.status==='done'||t.status==='failed';}).map(function(t){
    return '<div>'+t.createdAt+' &middot; '+escHtml(t.label||t.type||'Task')+' &middot; '+t.status.toUpperCase()+'</div>';
  }).join('')||'No completed runs yet.';
  if(typeof v7RenderWorkflows==='function')v7RenderWorkflows();
  if(typeof v7RenderMachineSparkline==='function')v7RenderMachineSparkline();
}
function v7SetTeamTab(tab,el){
  document.querySelectorAll('#v-command .v7-tab').forEach(function(t){t.classList.remove('active');});
  if(el)el.classList.add('active');
  document.getElementById('v7-team-floor').style.display=tab==='floor'?'grid':'none';
  document.getElementById('v7-team-queue').style.display=tab==='queue'?'block':'none';
  document.getElementById('v7-team-workflows').style.display=tab==='workflows'?'block':'none';
  document.getElementById('v7-team-logs').style.display=tab==='logs'?'block':'none';
  v7RenderTeamTab();
}

/* ---- Suggested Commands (Command Center) ---- */
var V7_SUGGESTED=[
  {label:'Find 20 leads in Ahmedabad',mode:'plan'},
  {label:'Draft follow-up for pending leads',mode:'outreach'},
  {label:'Generate this week\u2019s proposal',mode:'proposal'},
  {label:'Give me today\u2019s briefing',mode:'brief'}
];
function v7RenderSuggestedCommands(){
  var el=document.getElementById('v7-suggested-commands');
  if(!el)return;
  el.innerHTML=V7_SUGGESTED.map(function(s,i){return '<button type="button" class="v7-cmd-chip" onclick="v7RunSuggestedCommand('+i+')">'+escHtml(s.label)+'</button>';}).join('');
}
function v7RunSuggestedCommand(i){
  var s=V7_SUGGESTED[i];if(!s)return;
  var pill=document.querySelector('.mode-pill[data-mode="'+s.mode+'"]');
  if(pill)setMode(s.mode,pill);
  var inp=document.getElementById('chat-inp');
  if(inp){inp.value=s.label;autoResize(inp);inp.focus();}
}

/* ---- Clients summary table (reference-style Plan/Revenue/Next Action) ---- */
function v7RenderClientsSummary(){
  var tbody=document.getElementById('v7-clients-summary-tbody');
  if(!tbody||typeof clients==='undefined')return;
  var scls={active:'v7-status-active',onboarding:'v7-status-onboarding',negotiation:'v7-status-negotiation'};
  tbody.innerHTML=clients.map(function(c){
    var next=(c.tasks||[]).find(function(t){return !t.done;});
    return '<tr>'
      +'<td>'+escHtml(c.name)+'</td>'
      +'<td><span class="v7-status-pill '+(scls[c.status]||'v7-status-onboarding')+'">'+escHtml(c.status||'').toUpperCase()+'</span></td>'
      +'<td>'+escHtml(c.tier||'\u2014')+'</td>'
      +'<td>\u20B9'+(c.amount||0).toLocaleString('en-IN')+'</td>'
      +'<td>'+(next?escHtml(next.text):'\u2014')+'</td>'
      +'</tr>';
  }).join('');
}

/* =========================================================
   YUVI COMMAND CENTER — right-panel chat thread (reference image 2).
   Read-only mirror of the existing chatHistory array (does not alter
   chatHistory/sendChat). Timestamps are captured locally the moment
   each message is first observed here (chatHistory itself has none),
   so they reflect real send time, not fabricated history.
   Inline result cards are derived from real local state at render
   time (task-queue entries, health-score sub-scores) keyed off
   simple keyword matches in the assistant's own real reply text —
   nothing invented if no match is found.
   ========================================================= */
var V7_CHAT_TS=[];
function v7RenderChatThread(){
  var wrap=document.getElementById('yuvi-canvas-wrap');
  var empty=document.getElementById('yuvi-canvas-empty');
  if(!wrap||typeof chatHistory==='undefined')return;
  var thread=document.getElementById('v7-chat-thread');
  if(!thread){
    thread=document.createElement('div');
    thread.id='v7-chat-thread';
    thread.className='v7-chat-thread';
    wrap.insertBefore(thread,empty?empty.nextSibling:wrap.firstChild);
  }
  while(V7_CHAT_TS.length<chatHistory.length)V7_CHAT_TS.push(new Date().toISOString());
  if(empty)empty.style.display=chatHistory.length?'none':'';
  var q=v7GetQueue();
  thread.innerHTML=chatHistory.map(function(m,i){
    var t=new Date(V7_CHAT_TS[i]||Date.now()).toLocaleTimeString('en-IN',{hour:'2-digit',minute:'2-digit'});
    var who=m.role==='user'?'You':'YUVI';
    var card='';
    if(m.role==='assistant'){
      var lastTask=q[q.length-1];
      if(/mission accepted|scout has started|finding|hunting/i.test(m.content)&&lastTask){
        card='<div class="v7-inline-card"><div class="v7-inline-card-lbl">MISSION STARTED</div><div class="v7-inline-card-body">'+escHtml(lastTask.label||lastTask.type||'Task')+'</div><div class="v7-inline-card-bar"><div style="width:'+(lastTask.status==='done'?100:45)+'%;"></div></div></div>';
      }else if(/audit|overall score/i.test(m.content)){
        card='<div class="v7-inline-card"><div class="v7-inline-card-lbl">AUDIT SCORE</div><div class="v7-inline-card-body">Overall '+v7ComputeHealthScore()+'/100</div></div>';
      }else if(/carousel|instagram|content.*(created|ready)/i.test(m.content)){
        card='<div class="v7-inline-card"><div class="v7-inline-card-lbl">CONTENT CREATED</div><div class="v7-inline-card-body">Ready for review</div></div>';
      }
    }
    return '<div class="v7-chat-msg'+(m.role==='user'?' user':'')+'"><div class="v7-chat-msg-head"><span>'+who+'</span><span class="v7-chat-msg-time">'+t+'</span></div><div class="v7-chat-msg-body">'+escHtml((m.content||'').substring(0,600))+'</div>'+card+'</div>';
  }).join('');
  thread.scrollTop=thread.scrollHeight;
}
setInterval(v7RenderChatThread,1200);

/* =========================================================
   YUVI COMMAND CENTER — hero stats, mission wheel, active
   missions, team status, tracker/radar/calendar, sysbar.
   All derived from real v7GetQueue()/leads/clients/revenueData/
   v7ComputeHealthScore() — no fabricated numbers.
   ========================================================= */
function v7RenderCCHero(){
  var g=document.getElementById('cc-greet');
  if(g){var h=new Date().getHours();g.textContent='Good '+(h<12?'Morning':h<17?'Afternoon':'Evening')+', Shlok.';}
  var q=v7GetQueue();
  var contentDone=q.filter(function(t){return t.status==='done';}).length;
  var monthRev=revenueData.reduce(function(s,r){return s+(r.amount||0);},0);
  var stats=[
    {lbl:'Active Missions',val:q.filter(function(t){return t.status==='queued'||t.status==='working';}).length},
    {lbl:'Leads Generated',val:leads.length},
    {lbl:'Content Created',val:contentDone},
    {lbl:'Revenue This Month',val:'\u20B9'+monthRev.toLocaleString('en-IN')}
  ];
  var el=document.getElementById('v7-cc-stats');
  if(el)el.innerHTML=stats.map(function(s){return '<div class="v7-cc-stat"><div class="v7-cc-stat-val">'+s.val+'</div><div class="v7-cc-stat-lbl">'+s.lbl.toUpperCase()+'</div></div>';}).join('');
}
function v7RenderMissionWheel(){
  var q=v7GetQueue();
  var running=q.filter(function(t){return t.status==='working';}).length;
  var queued=q.filter(function(t){return t.status==='queued'||t.status==='awaiting_approval';}).length;
  var done=q.filter(function(t){return t.status==='done';}).length;
  var failed=q.filter(function(t){return t.status==='failed';}).length;
  var total=running+queued+done+failed||1;
  var wheel=document.getElementById('v7-mission-wheel');
  if(wheel){
    wheel.style.setProperty('--v7-run',Math.round(running/total*100));
    wheel.style.setProperty('--v7-queued',Math.round(queued/total*100));
    wheel.style.setProperty('--v7-done',Math.round(done/total*100));
  }
  var runEl=document.getElementById('v7-mw-running');if(runEl)runEl.textContent=running;
  var leg=document.getElementById('v7-mission-legend');
  if(leg)leg.innerHTML=[['Running',running,'var(--green)'],['Queued',queued,'var(--gold)'],['Completed',done,'var(--blade3)'],['Failed',failed,'var(--red,#e05252)']].map(function(r){return '<div><span style="background:'+r[2]+';"></span>'+r[0]+': '+r[1]+'</div>';}).join('');
}
function v7RenderActiveMissionsList(){
  var el=document.getElementById('v7-active-missions');
  if(!el)return;
  var q=v7GetQueue().filter(function(t){return t.status==='queued'||t.status==='working'||t.status==='awaiting_approval';}).slice(-6).reverse();
  el.innerHTML=q.length?q.map(function(t){
    var agent=V7_AGENTS.find(function(a){return a.id===t.agent;});
    return '<div class="v7-active-mission-row"><span>'+escHtml(t.label||t.type||'Task')+(agent?' \u00B7 '+agent.name:'')+'</span><span class="v7-mission-pill '+(t.status==='working'?'running':'queued')+'">'+t.status.toUpperCase().replace('_',' ')+'</span></div>';
  }).join(''):'<div style="font-family:var(--mono);font-size:8px;color:var(--muted);">No active missions.</div>';
}
function v7RenderTeamStatusCompact(){
  var el=document.getElementById('v7-team-status-compact');
  if(!el)return;
  var q=v7GetQueue();
  el.innerHTML=V7_AGENTS.map(function(a){
    var tasks=q.filter(function(t){return t.agent===a.id&&(t.status==='queued'||t.status==='working');});
    var status=tasks.length?'working':'idle';
    return '<div class="v7-team-status-row"><span><span class="v7-team-status-dot '+status+'"></span>'+a.name+'</span><span style="color:var(--muted);font-family:var(--mono);font-size:8px;">'+status.toUpperCase()+'</span></div>';
  }).join('');
}
function v7RenderLeadGenTracker(){
  var el=document.getElementById('v7-lead-gen-tracker');
  if(!el)return;
  var buckets={};
  leads.forEach(function(l){var d=new Date(l.id&&l.id>1e12?l.id:Date.now());var k=d.toISOString().slice(0,10);buckets[k]=(buckets[k]||0)+1;});
  var keys=Object.keys(buckets).sort().slice(-7);
  if(!keys.length){el.innerHTML='<div style="font-family:var(--mono);font-size:8px;color:var(--muted);">No lead data yet.</div>';return;}
  var max=Math.max.apply(null,keys.map(function(k){return buckets[k];}))||1;
  var w=260,h=70,step=w/(Math.max(1,keys.length-1));
  var pts=keys.map(function(k,i){return (i*step).toFixed(1)+','+(h-(buckets[k]/max)*h).toFixed(1);}).join(' ');
  el.innerHTML='<svg viewBox="0 0 '+w+' '+h+'" style="width:100%;height:70px;"><polyline points="'+pts+'" fill="none" stroke="var(--v7-violet,#a78bfa)" stroke-width="2"/></svg><div style="font-family:var(--mono);font-size:7px;color:var(--muted);margin-top:4px;">'+leads.length+' total leads</div>';
}
function v7RenderAuditRadar(){
  var el=document.getElementById('v7-audit-radar');
  if(!el)return;
  var totalClients=clients.length,activeClients=clients.filter(function(c){return c.status==='active';}).length;
  var clientHealth=totalClients?activeClients/totalClients:0.7;
  var totalLeads=leads.length,converted=leads.filter(function(l){return l.status==='interested'||l.status==='closed';}).length;
  var leadConv=totalLeads?converted/totalLeads:0.7;
  var totalRev=revenueData.reduce(function(s,r){return s+(r.amount||0);},0),paid=revenueData.filter(function(r){return r.status==='paid';}).reduce(function(s,r){return s+(r.amount||0);},0);
  var revColl=totalRev?paid/totalRev:0.7;
  var totalRows=revenueData.length,overdue=revenueData.filter(function(r){return r.status==='overdue';}).length;
  var payRel=totalRows?1-(overdue/totalRows):0.7;
  var axes=[['Client Health',clientHealth],['Lead Conversion',leadConv],['Revenue Collection',revColl],['Payment Reliability',payRel]];
  var cx=90,cy=80,r=60,n=axes.length;
  var pts=axes.map(function(a,i){var ang=(Math.PI*2*i/n)-Math.PI/2;var rad=r*a[1];return (cx+rad*Math.cos(ang)).toFixed(1)+','+(cy+rad*Math.sin(ang)).toFixed(1);}).join(' ');
  var grid=[0.33,0.66,1].map(function(f){return '<polygon points="'+axes.map(function(a,i){var ang=(Math.PI*2*i/n)-Math.PI/2;return (cx+r*f*Math.cos(ang)).toFixed(1)+','+(cy+r*f*Math.sin(ang)).toFixed(1);}).join(' ')+'" fill="none" stroke="var(--edge2)" stroke-width="1"/>';}).join('');
  var labels=axes.map(function(a,i){var ang=(Math.PI*2*i/n)-Math.PI/2;var lx=cx+(r+22)*Math.cos(ang),ly=cy+(r+14)*Math.sin(ang);return '<text x="'+lx.toFixed(1)+'" y="'+ly.toFixed(1)+'" font-size="7" fill="var(--muted)" text-anchor="middle" font-family="monospace">'+a[0]+'</text>';}).join('');
  el.innerHTML='<div class="v7-radar-wrap"><svg viewBox="0 0 180 160">'+grid+'<polygon points="'+pts+'" fill="rgba(167,139,250,.28)" stroke="var(--v7-violet,#a78bfa)" stroke-width="1.5"/>'+labels+'</svg></div><div style="font-family:var(--mono);font-size:7px;color:var(--muted);margin-top:2px;">Overall '+v7ComputeHealthScore()+'/100 \u2014 axes are the real Business Health sub-scores (no separate SEO/social audit data exists yet)</div>';
}
function v7ContentCategoryCounts(){
  var q=v7GetQueue();
  var labels={content_batch:'Content Batch',lead_gen:'Lead Gen',competitor_research:'Research',monthly_report:'Reports',client_checkin:'Client Ops'};
  var counts={};
  q.forEach(function(t){var k=labels[t.type]||'Other';counts[k]=(counts[k]||0)+1;});
  return counts;
}
function v7RenderContentCalendar(){
  var el=document.getElementById('v7-content-calendar');
  if(!el)return;
  var counts=v7ContentCategoryCounts();
  var keys=Object.keys(counts);
  if(!keys.length){el.innerHTML='<div style="font-family:var(--mono);font-size:8px;color:var(--muted);">No content-related tasks queued yet.</div>';return;}
  el.innerHTML=keys.map(function(k){return '<div class="v7-active-mission-row"><span>'+k+'</span><span style="font-family:var(--mono);font-size:9px;color:var(--text);">'+counts[k]+'</span></div>';}).join('');
}
function v7RenderSysBar(){
  var el=document.getElementById('v7-sysbar');
  if(!el)return;
  var q=v7GetQueue();
  var done=q.filter(function(t){return t.status==='done';}).length;
  var failed=q.filter(function(t){return t.status==='failed';}).length;
  var successRate=(done+failed)?Math.round(done/(done+failed)*100):100;
  var lastSync=localStorage.getItem('yuvi_workspace_online_since');
  el.innerHTML=''
    +'<div><b>'+(v7WorkspaceOnline?'ONLINE':'OFFLINE')+'</b>n8n Backend</div>'
    +'<div><b>'+q.filter(function(t){return t.status==='queued'||t.status==='awaiting_approval';}).length+'</b>Waiting</div>'
    +'<div><b>'+done+'</b>Executions Today</div>'
    +'<div><b>'+successRate+'%</b>Success Rate</div>'
    +'<div><b>'+(lastSync?timeAgo(lastSync):'\u2014')+'</b>Last Sync</div>';
}
function v7RenderCommandCenter(){
  v7RenderCCHero();
  v7RenderMissionWheel();
  v7RenderActiveMissionsList();
  v7RenderTeamStatusCompact();
  v7RenderLeadGenTracker();
  v7RenderAuditRadar();
  v7RenderContentCalendar();
  v7RenderSysBar();
  v7RenderChatThread();
}
document.addEventListener('DOMContentLoaded',function(){
  v7RenderCommandCenter();
  setInterval(v7RenderCommandCenter,15000);
});

/* =========================================================
   YUVI HOME DASHBOARD — metric row, agent overview, funnel,
   donut, opportunities, alerts, mission checklist. Additive,
   all bound to real clients/leads/revenueData/pipeline/
   v7GetQueue()/priorities. No fabricated numbers.
   ========================================================= */
function v7RenderMetricsRow(){
  var q=v7GetQueue();
  var missions=q.filter(function(t){return t.status==='queued'||t.status==='working';}).length;
  var mEl=document.getElementById('v7-metric-missions');if(mEl)mEl.textContent=missions;
  var tasksDone=(typeof priorities!=='undefined'?priorities.filter(function(p){return p.done;}).length:0)+q.filter(function(t){return t.status==='done';}).length;
  var tEl=document.getElementById('v7-metric-tasks-done');if(tEl)tEl.textContent=tasksDone;
  var spark=document.getElementById('v7-metric-missions-spark');
  if(spark){
    var now=Date.now(),HOUR=3600000,buckets=new Array(8).fill(0);
    q.forEach(function(t){var age=now-new Date(t.createdAt||now).getTime();var idx=7-Math.min(7,Math.floor(age/HOUR));if(idx>=0&&idx<8)buckets[idx]++;});
    var max=Math.max.apply(null,buckets)||1;var w=90,h=20,step=w/(buckets.length-1);
    var pts=buckets.map(function(v,i){return (i*step).toFixed(1)+','+(h-(v/max)*h).toFixed(1);}).join(' ');
    spark.innerHTML='<svg viewBox="0 0 '+w+' '+h+'" style="width:100%;height:20px;"><polyline points="'+pts+'" fill="none" stroke="var(--v7-violet,#a78bfa)" stroke-width="1.4"/></svg>';
  }
}
function v7RenderAgentOverview(){
  var container=document.getElementById('v7-agent-overview');
  if(!container)return;
  var q=v7GetQueue();
  container.innerHTML=V7_AGENTS.map(function(a){
    var tasks=q.filter(function(t){return t.agent===a.id&&(t.status==='queued'||t.status==='working');});
    var status=tasks.length?'working':'idle';
    var pct=tasks.length?Math.min(90,20+tasks.length*20):0;
    return '<div class="v7-agent-card">'
      ...
      +'<div class="v7-agent-bar"><div class="v7-agent-fill" style="width:'+pct+'%;"></div></div>'
      +'<div class="v7-agent-eta">'+(tasks.length?tasks.length+' task(s) queued':'no tasks queued')+'</div>'
      +'<button class="btn" style="width:100%;margin-top:6px;font-size:7px;padding:4px;" onclick="nav(\'command\')">VIEW DETAILS</button>'
      +'</div>';
  }).join('');
}
function v7RenderLeadFunnel(){
  var el=document.getElementById('v7-lead-funnel');
  if(!el)return;
  var stages=[
    ['New',leads.filter(function(l){return l.status==='new';}).length],
    ['Contacted',leads.filter(function(l){return l.status==='contacted';}).length],
    ['Qualified',leads.filter(function(l){return l.status==='interested';}).length],
    ['Proposal',pipeline.filter(function(p){return p.stage==='proposal_sent';}).length],
    ['Closed',leads.filter(function(l){return l.status==='closed';}).length+pipeline.filter(function(p){return p.stage==='closed';}).length]
  ];
  el.innerHTML=stages.map(function(s){return '<div class="v7-funnel-row"><div class="v7-funnel-bar"><b>'+s[1]+'</b>'+s[0]+'</div></div>';}).join('');
}
function v7RenderContentDonut(){
  var el=document.getElementById('v7-content-donut');
  if(!el)return;
  var counts=v7ContentCategoryCounts();
  var keys=Object.keys(counts);
  var colors=['#a78bfa','#34c759','#ffc400','#5aa9ff','#e05252'];
  if(!keys.length){el.innerHTML='<div style="font-family:var(--mono);font-size:8px;color:var(--muted);">No content-related tasks queued yet.</div>';return;}
  var total=keys.reduce(function(s,k){return s+counts[k];},0);
  var acc=0,stops=keys.map(function(k,i){var pct=counts[k]/total*100;var seg=colors[i%colors.length]+' '+acc.toFixed(1)+'% '+(acc+pct).toFixed(1)+'%';acc+=pct;return seg;}).join(',');
  el.innerHTML='<div class="v7-donut-wrap"><div class="v7-donut" style="background:conic-gradient('+stops+');"></div><div class="v7-donut-legend">'+keys.map(function(k,i){return '<div><span class="sw" style="background:'+colors[i%colors.length]+';"></span>'+k+' \u2014 '+counts[k]+'</div>';}).join('')+'</div></div>';
}
function v7RenderTopOpportunities(){
  var el=document.getElementById('v7-top-opportunities');
  if(!el)return;
  var top=leads.slice().sort(function(a,b){return (b.score||0)-(a.score||0);}).slice(0,5);
  if(!top.length){el.innerHTML='<div style="font-family:var(--mono);font-size:8px;color:var(--muted);">No leads yet.</div>';return;}
  el.innerHTML=top.map(function(l,i){
    var score=l.score||0;
    return '<div class="v7-opp-row"><span class="v7-opp-rank">'+(i+1)+'</span><div><div class="v7-opp-name">'+escHtml(l.name)+'</div><div class="v7-opp-sub">'+escHtml(l.address||'')+'</div></div><div class="v7-opp-score"><b>'+score+'</b><span class="v7-opp-tag '+(score>=8?'high':'med')+'">'+(score>=8?'HIGH':'MED')+'</span></div></div>';
  }).join('');
}
function v7GetAlerts(){
  var alerts=[];
  clients.forEach(function(c){if(c.payment==='overdue')alerts.push({icon:'\u26A0\uFE0F',text:'Payment overdue \u2014 '+c.name,time:Date.now()});});
  pipeline.forEach(function(p){if(p.stage==='proposal_sent')alerts.push({icon:'\uD83D\uDCC4',text:'Proposal pending review \u2014 '+p.name,time:new Date(p.stageEnteredAt||p.lastTouched||Date.now()).getTime()});});
  var q=v7GetQueue();
  q.filter(function(t){return t.status==='awaiting_approval';}).forEach(function(t){alerts.push({icon:'\u23F3',text:'Awaiting approval \u2014 '+(t.label||t.type),time:new Date(t.createdAt||Date.now()).getTime()});});
  alerts.sort(function(a,b){return b.time-a.time;});
  return alerts.slice(0,6);
}
function v7RenderAlerts(){
  var el=document.getElementById('v7-alerts-list');
  if(!el)return;
  var alerts=v7GetAlerts();
  el.innerHTML=alerts.length?alerts.map(function(a){return '<div class="v7-alert-row"><span class="v7-alert-icon">'+a.icon+'</span><div><div class="v7-alert-text">'+escHtml(a.text)+'</div><div class="v7-alert-time">'+timeAgo(new Date(a.time).toISOString())+'</div></div></div>';}).join(''):'<div style="font-family:var(--mono);font-size:8px;color:var(--muted);">No alerts \u2014 all clear.</div>';
}
function v7RenderMissionChecklist(){
  var el=document.getElementById('v7-mission-checklist');
  if(!el||typeof priorities==='undefined')return;
  el.innerHTML=priorities.length?priorities.map(function(p,i){
    return '<div class="v7-active-mission-row" style="cursor:pointer;" onclick="togglePriority('+i+')"><span>'+(p.done?'\u2611':'\u2610')+' '+escHtml(p.text)+'</span><span style="font-family:var(--mono);font-size:7px;color:var(--muted);">'+(p.done?'DONE':'PENDING')+'</span></div>';
  }).join(''):'<div style="font-family:var(--mono);font-size:8px;color:var(--muted);">No mission items \u2014 add priorities.</div>';
}
function v7RenderHomeDashboard(){
  v7RenderMetricsRow();
  v7RenderAgentOverview();
  v7RenderLeadFunnel();
  v7RenderContentDonut();
  v7RenderTopOpportunities();
  v7RenderAlerts();
  v7RenderMissionChecklist();
  var lbl=document.getElementById('v7-yuvi-status-label');
  if(lbl)lbl.textContent=v7WorkspaceOnline?'ONLINE':'ONLINE (LOCAL)';
}
document.addEventListener('DOMContentLoaded',function(){
  v7RenderHomeDashboard();
  setInterval(v7RenderHomeDashboard,15000);
});

/* ---- init (fires after existing app.js boot logic has run) ---- */
document.addEventListener('DOMContentLoaded',function(){
  v7RenderSuggestedCommands();
  v7RenderTeamTab();
  v7CheckWorkspace();
});
function saveN8nUrl(){
  var url=(document.getElementById('s-n8n-url')||{}).value||'';
  localStorage.setItem('yuvi_n8n_webhook_url',url.trim());
  if(typeof showToast==='function')showToast('Workspace URL saved.');
  v7CheckWorkspace();
}

/* =========================================================
   YUVI v7.1 — HOME DASHBOARD PANELS
   Renders the same real data (agents/queue, clients, pipeline)
   into extra preview panels on Home, matching the reference
   layout where AI Team / Command Center / Pipeline / Clients /
   Knowledge Base are all visible on one screen. Read-only
   previews — "OPEN"/"VIEW ALL" buttons still nav() to the full
   interactive view. No existing function bodies were changed.
   ========================================================= */
function v7RenderAgentGridInto(container){
  if(!container)return;
  var q=v7GetQueue();
  container.innerHTML=V7_AGENTS.map(function(a){
    var tasks=q.filter(function(t){return t.agent===a.id&&(t.status==='queued'||t.status==='working');});
...
function v7RenderSuggestedInto(chipsEl,historyEl){
  if(chipsEl)chipsEl.innerHTML=V7_SUGGESTED.map(function(s,i){return '<button type="button" class="v7-cmd-chip" onclick="nav(\'command\');setTimeout(function(){v7RunSuggestedCommand('+i+');},50);">'+escHtml(s.label)+'</button>';}).join('');
  if(historyEl){
    var q=v7GetQueue().slice(-4).reverse();
    historyEl.innerHTML=q.length?q.map(function(t){return '<div class="v7-cmd-history-item"><span>'+escHtml(t.label||t.type||'Task')+'</span><span class="v7-ch-status">'+t.status.toUpperCase().replace('_',' ')+'</span></div>';}).join(''):'<div style="font-family:var(--mono);font-size:8px;color:var(--muted);">No recent commands.</div>';
  }
}
function v7RenderPipelineSummary(){
  var el=document.querySelector('.v7-pipeline-summary');
  if(!el||typeof PIPE_STAGES==='undefined'||typeof pipeline==='undefined')return;
  el.innerHTML=PIPE_STAGES.map(function(stage){
    var sc=pipeline.filter(function(c){return c.stage===stage;});
    var val=sc.reduce(function(sum,c){var m=(c.service||'').match(/[\d,]+/);return sum+(m?parseInt(m[0].replace(/,/g,'')):0);},0);
    return '<div class="v7-pipe-stage-row"><span class="v7-pipe-stage-name">'+(PIPE_LABELS[stage]||stage)+'</span><span class="v7-pipe-stage-count">'+sc.length+' leads</span><span class="v7-pipe-stage-val">\u20B9'+val.toLocaleString('en-IN')+'</span></div>';
  }).join('');
}
function v7RenderClientsSummaryInto(tbody){
  if(!tbody||typeof clients==='undefined')return;
  var scls={active:'v7-status-active',onboarding:'v7-status-onboarding',negotiation:'v7-status-negotiation'};
  tbody.innerHTML=clients.map(function(c){
    var next=(c.tasks||[]).find(function(t){return !t.done;});
    return '<tr><td>'+escHtml(c.name)+'</td><td><span class="v7-status-pill '+(scls[c.status]||'v7-status-onboarding')+'">'+escHtml(c.status||'').toUpperCase()+'</span></td><td>'+escHtml(c.tier||'\u2014')+'</td><td>\u20B9'+(c.amount||0).toLocaleString('en-IN')+'</td><td>'+(next?escHtml(next.text):'\u2014')+'</td></tr>';
  }).join('');
}
function v7RenderKnowledgeGrid(){
  var el=document.querySelector('.v7-knowledge-grid');
  if(!el)return;
  var cats=typeof knowledgeFiles!=='undefined'?knowledgeFiles:[];
  var counts={};
  cats.forEach(function(f){var c=f.category||'General';counts[c]=(counts[c]||0)+1;});
  var labels=Object.keys(counts).length?Object.keys(counts):['Business Strategy','Client Research','Proposals','Ideas'];
  el.innerHTML=labels.map(function(l){
    return '<div class="v7-know-card" onclick="nav(\'library\')"><div class="v7-know-title">'+escHtml(l)+'</div><div class="v7-know-meta">'+(counts[l]||0)+' item(s)</div></div>';
  }).join('');
}
function v7RenderHomePanels(){
  v7RenderAgentGridInto(document.querySelector('.v7-agent-grid-home'));
  v7RenderSuggestedInto(document.querySelector('.v7-cmd-suggest-home'),document.querySelector('.v7-cmd-history-home'));
  v7RenderPipelineSummary();
  v7RenderClientsSummaryInto(document.querySelector('.v7-clients-summary-tbody-home'));
  v7RenderKnowledgeGrid();
}
/* Refresh home panels whenever core data renders happen, without touching
   the original functions' bodies. */
if(typeof renderClients==='function'){
  var __v7_origRenderClients=renderClients;
  renderClients=function(){__v7_origRenderClients.apply(this,arguments);v7RenderClientsSummaryInto(document.querySelector('.v7-clients-summary-tbody-home'));};
}
if(typeof renderPipeline==='function'){
  var __v7_origRenderPipeline=renderPipeline;
  renderPipeline=function(){__v7_origRenderPipeline.apply(this,arguments);v7RenderPipelineSummary();if(typeof v7RenderAlerts==='function')v7RenderAlerts();};
}
if(typeof renderPriorities==='function'){
  var __v7_origRenderPriorities=renderPriorities;
  renderPriorities=function(){__v7_origRenderPriorities.apply(this,arguments);if(typeof v7RenderMissionChecklist==='function')v7RenderMissionChecklist();};
}
if(typeof renderRevenue==='function'){
  var __v7_origRenderRevenue2=renderRevenue;
  renderRevenue=function(){__v7_origRenderRevenue2.apply(this,arguments);if(typeof v7RenderMetricsRow==='function')v7RenderMetricsRow();};
}
document.addEventListener('DOMContentLoaded',function(){
  v7RenderHomePanels();
  setInterval(v7RenderHomePanels,15000);
});

/* =========================================================
   YUVI v7.1 — SETTINGS REDESIGN (organized tabs)
   Does not touch any field IDs, save handlers, or logic inside
   settings — only wraps the existing 7 sections into a tabbed
   layout so it reads as organized instead of one long scroll.
   ========================================================= */
var V7_SETT_TABS=[
  {id:'api',label:'API & AI'},
  ...
  {id:'yuvi',label:'YUVI Identity'},
  {id:'workspace',label:'Workspace (n8n)'}
];
function v7InitSettingsTabs(){
  var panel=document.getElementById('settings-panel');
  if(!panel||document.getElementById('v7-sett-tabs'))return; // already initialized
  var sections=panel.querySelectorAll('.sett-sec');
  if(sections.length<7)return; // structure not as expected, bail safely
  var tabBar=document.createElement('div');
  tabBar.className='v7-sett-tabs';
  tabBar.id='v7-sett-tabs';
  tabBar.innerHTML=V7_SETT_TABS.map(function(t,i){return '<div class="v7-sett-tab'+(i===0?' active':'')+'" data-tab="'+t.id+'" onclick="v7SetSettTab(\''+t.id+'\')">'+t.label+'</div>';}).join('');
  sections[0].parentNode.insertBefore(tabBar,sections[0]);
  var lastPage=tabBar;
  V7_SETT_TABS.forEach(function(t,i){
    var page=document.createElement('div');
    page.className='v7-sett-page'+(i===0?' active':'');
    page.id='v7-sett-page-'+t.id;
    var start=sections[i];
    var end=sections[i+1]||null;
    var node=start;
    var toMove=[];
    while(node&&node!==end){var next=node.nextSibling;toMove.push(node);node=next;}
    toMove.forEach(function(n){page.appendChild(n);});
    lastPage.after(page);
    lastPage=page;
  });
}
function v7SetSettTab(id){
  document.querySelectorAll('.v7-sett-tab').forEach(function(t){t.classList.toggle('active',t.dataset.tab===id);});
  document.querySelectorAll('.v7-sett-page').forEach(function(p){p.classList.toggle('active',p.id==='v7-sett-page-'+id);});
}
var __v7_origOpenSettings=typeof openSettings==='function'?openSettings:null;
if(__v7_origOpenSettings){
  openSettings=function(){__v7_origOpenSettings.apply(this,arguments);v7InitSettingsTabs();};
}

/* =========================================================
   YUVI v7.4 — BUSINESS HEALTH SCORE (item 2)
   Weighted formula (documented here + in REGRESSION_CHECK_v7.md):
     30% — active client ratio      = activeClients / totalClients
     25% — lead conversion rate     = (interested+closed leads) / totalLeads
     25% — revenue collection rate  = paid revenue / total revenue (revenueData)
     20% — overdue penalty          = 1 - (overdueCount / totalRevenueRows)
   Any component with no data defaults to a neutral 0.7 (70%) rather than
   0, so an empty dashboard doesn't read as "business is failing".
   Score = round(weighted sum * 100), clamped 0-100.
   ========================================================= */
function v7ComputeHealthScore(){
  var totalClients=clients.length;
  var activeClients=clients.filter(function(c){return c.status==='active';}).length;
  var clientHealth=totalClients?activeClients/totalClients:0.7;

  var totalLeads=leads.length;
  var convertedLeads=leads.filter(function(l){return l.status==='interested'||l.status==='closed';}).length;
  var leadConversion=totalLeads?convertedLeads/totalLeads:0.7;

  var totalRevenue=revenueData.reduce(function(s,r){return s+(r.amount||0);},0);
  var paidRevenue=revenueData.filter(function(r){return r.status==='paid';}).reduce(function(s,r){return s+(r.amount||0);},0);
  var revenueCollection=totalRevenue?paidRevenue/totalRevenue:0.7;

  var totalRows=revenueData.length;
  var overdueRows=revenueData.filter(function(r){return r.status==='overdue';}).length;
  var overduePenalty=totalRows?1-(overdueRows/totalRows):0.7;

  var weighted=(clientHealth*0.30)+(leadConversion*0.25)+(revenueCollection*0.25)+(overduePenalty*0.20);
  return Math.max(0,Math.min(100,Math.round(weighted*100)));
}
function v7ComputeAndRenderHealthScore(){
  var score=v7ComputeHealthScore();
  var scoreEl=document.getElementById('v7-health-score');
  if(scoreEl)scoreEl.textContent=score;
  var ring=document.getElementById('v7-health-ring');
  if(ring)ring.style.setProperty('--v7-pct',score);
  return score;
}
