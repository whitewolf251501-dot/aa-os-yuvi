...
(function () {
  ...
  // v6.1 — TRACK B STEP 3: shared crash-safe read for the raw JSON.parse(localStorage...) calls below.
  function safeReadLS(key, fallback) { try { return JSON.parse(localStorage.getItem(key) || JSON.stringify(fallback)); } catch (e) { return fallback; } }
  ...
  function extractClient(message) {
    const clients = safeReadLS('yuvi_clients', []);
  ...
  async function executePromptSkill(ps, capability, args={}) {
    ...
    const skillMem = safeReadLS(memKey, []);
    skillMem.unshift({
  ...
  function getSkillMemory(skill_id) {
    const all = safeReadLS('yuvi_skill_memory', []);
