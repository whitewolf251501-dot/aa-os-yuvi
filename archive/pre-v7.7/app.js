...
function nav(viewId,navEl,mobileEl){
  ...
  if(window.__v7RainResize)window.__v7RainResize();
}
...
function v7RenderTeamTab(){
  ...
  floor.querySelectorAll('.v7-agent-card').forEach(function(card,i){card.style.cursor='pointer';card.onclick=function(){v7OpenAgentModal(V7_AGENTS[i].id);};});
...
function v7RenderAgentGridInto(container){
  ...
  container.querySelectorAll('.v7-agent-card').forEach(function(card,i){card.style.cursor='pointer';card.onclick=function(){v7OpenAgentModal(V7_AGENTS[i].id);};});
}
...
if(typeof renderPipeline==='function'){
  ...
  renderPipeline=function(){__v7_origRenderPipeline.apply(this,arguments);v7RenderPipelineSummary();};
}
document.addEventListener('DOMContentLoaded',function(){
  v7RenderHomePanels();
...
/* =========================================================
   YUVI v7.2 — AGENT DETAIL MODAL (tap card: today/yesterday/tomorrow)
   Derived entirely from real local task-queue history
   (v7GetQueue()) grouped by date + agent. No fake data — if an
   agent has no logged tasks for a day, that section says so.
   ========================================================= */
function v7DateKey(d){var dt=new Date(d);return dt.toISOString().slice(0,10);}
function v7OpenAgentModal(agentId){
  var agent=V7_AGENTS.find(function(a){return a.id===agentId;});
  if(!agent)return;
  var q=v7GetQueue().filter(function(t){return t.agent===agentId;});
  var today=v7DateKey(new Date());
  var y=new Date();y.setDate(y.getDate()-1);var yesterday=v7DateKey(y);
  var doneToday=q.filter(function(t){return v7DateKey(t.createdAt)===today&&(t.status==='done'||t.status==='working');});
  var doneYesterday=q.filter(function(t){return v7DateKey(t.createdAt)===yesterday&&t.status==='done';});
  var upcoming=q.filter(function(t){return t.status==='queued'||t.status==='awaiting_approval';});
  function list(items,emptyMsg){
    if(!items.length)return '<div class="v7-am-empty">'+emptyMsg+'</div>';
    return items.map(function(t){return '<div class="v7-am-item">'+escHtml(t.label||t.type||'Task')+'</div>';}).join('');
  }
  var box=document.getElementById('v7-agent-modal-box');
  box.innerHTML='<div class="v7-am-head"><div class="v7-am-avatar">'+agent.name.charAt(0)+'</div><div><div class="v7-am-title">'+agent.name+'</div><div class="v7-am-role">'+agent.role.toUpperCase()+'</div></div></div>'
    +'<div class="v7-am-section-lbl">TODAY</div>'+list(doneToday,'Nothing logged yet today.')
    +'<div class="v7-am-section-lbl">YESTERDAY</div>'+list(doneYesterday,'No completed tasks logged for yesterday.')
    +'<div class="v7-am-section-lbl">UPCOMING / TOMORROW</div>'+list(upcoming,'No tasks queued ahead.')
    +'<button class="btn" style="width:100%;margin-top:14px;" onclick="v7CloseAgentModal()">CLOSE</button>';
  document.getElementById('v7-agent-modal').classList.add('show');
}
function v7CloseAgentModal(){document.getElementById('v7-agent-modal').classList.remove('show');}

/* =========================================================
   YUVI v7.2 — LEFT TRANSCRIPT LOG (Command view)
   Mirrors the real chatHistory array (already populated by the
   existing sendChat() for both typed and voice-dictated input,
   since voice just fills the same input box before sending).
   Read-only — does not alter chatHistory or sendChat.
   ========================================================= */
var __v7_lastTranscriptLen=0;
function v7RenderTranscriptLog(){
  var el=document.getElementById('v7-transcript-list');
  if(!el||typeof chatHistory==='undefined')return;
  if(chatHistory.length===__v7_lastTranscriptLen)return;
  __v7_lastTranscriptLen=chatHistory.length;
  el.innerHTML=chatHistory.map(function(m){
    return '<div class="v7-transcript-item'+(m.role==='user'?' user':'')+'"><span class="v7-tr-role">'+(m.role==='user'?'YOU':'YUVI')+'</span>'+escHtml((m.content||'').substring(0,220))+(m.content&&m.content.length>220?'\u2026':'')+'</div>';
  }).join('');
  el.scrollTop=el.scrollHeight;
}
setInterval(v7RenderTranscriptLog,1200);

/* =========================================================
   YUVI v7.2/v7.4 — DIGITAL RAIN BACKGROUND (canvas, additive layer)
   Lives inside #v-command > .v7-cmd-split, behind .v7-blank-canvas only.
   Every other view keeps the original bg-a/bg-g/bg-s dark background.
   Pure visual effect, no interaction, throttled to ~20fps, sized to
   its own container (not the full window), and paused whenever the
   Command view isn't the active view OR the tab is hidden.
   ========================================================= */
(function(){
  var canvas=document.getElementById('v7-rain-canvas');
  if(!canvas)return;
  var ctx=canvas.getContext('2d');
  var cols=[],fontSize=16,lastFrame=0;
  var chars='01YUVI$#@%&*+-<>';
  function resize(){
    var box=canvas.parentElement?canvas.parentElement.getBoundingClientRect():{width:window.innerWidth,height:window.innerHeight};
    canvas.width=Math.max(1,Math.round(box.width));
    canvas.height=Math.max(1,Math.round(box.height));
    var n=Math.floor(canvas.width/fontSize);
    cols=new Array(n).fill(0).map(function(){return Math.random()*-100;});
  }
  window.__v7RainResize=resize;
  if(typeof window.__v7CommandActive==='undefined')window.__v7CommandActive=false; // Home is the default view on boot
  function draw(ts){
    if(document.visibilityState==='hidden'||!window.__v7CommandActive){requestAnimationFrame(draw);return;}
    if(ts-lastFrame<50){requestAnimationFrame(draw);return;} // ~20fps cap
    lastFrame=ts;
    ...
    for(var i=0;i<cols.length;i++){
      ...
      ctx.fillStyle=Math.random()<0.05?'rgba(230,220,255,0.9)':'rgba(167,139,250,0.55)';
      ctx.fillText(ch,x,y);
